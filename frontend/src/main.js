...
function isSameOriginRendererInPagesBuild() {
  if (!IS_PAGES_BUILD) return false;
  const base = String(RENDER_BASE_URL || '').trim();
  if (!base) return false;
  try {
    // If base is relative, it resolves to the current origin.
    const resolved = new URL(base, window.location.origin);
    return resolved.origin === window.location.origin;
  } catch {
    // Non-URL strings (like "/renderer") are treated as same-origin.
    return true;
  }
}

const RENDERER_MISSING = IS_PAGES_BUILD && !String(RENDER_BASE_URL || '').trim();
const RENDERER_SAME_ORIGIN_STATIC = isSameOriginRendererInPagesBuild();
const NEEDS_RENDERER_BACKEND = RENDERER_MISSING || RENDERER_SAME_ORIGIN_STATIC;
...
function renderImageBlock({ label, imgAlt, imgSrc }) {
  const help = el('p', {
    ...
    text: NEEDS_RENDERER_BACKEND
      ? 'Images require a separate PHP renderer (not Workers). Set VITE_RENDER_URL to that backend and redeploy.'
      ...
      (NEEDS_RENDERER_BACKEND)
        ? el('div', { class: 'card warning', style: 'margin-bottom: 12px;' }, [
            ...
            el('div', { class: 'card-body' }, [
              ...
              RENDERER_SAME_ORIGIN_STATIC ? el('p', { class: 'muted', text: 'Note: VITE_RENDER_URL currently points to this same static site, which cannot generate images.' }) : null
            ])
          ])
        : null,
      !project
